...
We will see a quick setup to configure the ``healthchecks`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``healthchecks`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``healthchecks`` formulas under ``/srv/salt/healthchecks/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/healthchecks/parameters/dns:domain/
    ln -s dns:domain /srv/salt/healthchecks/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/healthchecks/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/healthchecks/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``healthchecks`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``healthchecks`` formulas under ``/srv/salt/healthchecks/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/healthchecks/parameters/roles
...
We create a configuration for the role ``healthchecks/server`` in ``/srv/salt/healthchecks/parameters/roles/healthchecks/server.yaml``:
...
We create another configuration for the role ``healthchecks/client`` in ``/srv/salt/healthchecks/parameters/roles/healthchecks/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles healthchecks/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles healthchecks/client
...
.. code-block:: console
    ...
    mv /srv/salt/healthchecks/parameters/dns:domain/  '/srv/salt/healthchecks/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-healthchecks-as-json:
      test.nop:
...
Here is an example based on `template-formula/healthchecks/config/file.sls`_:
    ...
    healthchecks-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='healthchecks-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ healthchecks.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``healthchecks`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ healthchecks['winner'] }}
...
.. _template-formula/healthchecks/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/healthchecks/config/file.sls
